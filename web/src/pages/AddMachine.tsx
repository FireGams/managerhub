...
export default function AddMachine() {
  ...
  function copyCmd() {
    if (!newToken) return
    const cmd = `curl -fsSL https://raw.githubusercontent.com/FireGams/managerhub/refs/heads/main/scripts/install.sh?v=1 | bash -s -- --controller ${controllerURL} --token ${newToken} --name \"${newLabel || 'my-machine'}\"`
...
{`curl -fsSL https://raw.githubusercontent.com/FireGams/managerhub/refs/heads/main/scripts/install.sh?v=1 | bash -s -- --controller ${controllerURL} --token ${newToken} --name \"${newLabel || 'my-machine'}\"`}
            </pre>
